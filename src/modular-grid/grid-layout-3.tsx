...
function classifyScreen(
  ...
  if (windowWidth < config.tabletScreen.minGridWidth) return "mobile";
  if (windowWidth < config.desktopScreen.minGridWidth) return "tablet";
  return "desktop";
}
...
function initGrid(
  ...
  switch (screenClass) {
    ...
    case "tablet": {
      const sc = config.tabletScreen;
      const gridMargin = resolveMargin(sc);
      const maxGridWidth = sc.maxGridWidth ?? window.width;
      // Tablet: clamp BOTH min and max
      const clampedGridWidth = Math.min(
        Math.max(sc.minGridWidth, window.width),
        maxGridWidth,
      );
...
export function GridLayout3Provider({
  config,
  children,
}: {
  config: LayoutConfig;
...
export function useScreenClass(): ScreenClass {
  ...
  if (ctx === null) {
    throw new Error(
      "useScreenClass must be used within a GridLayout3Provider",
    );
  }
  return ctx;
}
